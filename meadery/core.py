...
ACID_ADJUSTMENTS = {
    ...
    'lactic': AcidAddition(ta_effect=75.04 / 90.08,
                           components=[(1.0, 3.86, 90.08)]),
...
class Must:
    ...
    def abv(self, fg, method: str=DEFAULT_ABV) -> float:
        ...
        if fg >= og:
            ...
            return (og - fg) / 0.00727
        elif method == 'hall':
            return 76.08 * ((og - fg) / (1.775 - og)) * (fg / 0.78924)
        elif method == 'duncan':
    ...
    def ph_dilution_correction(self, parts_water: int, strip_ph: float, tol: float = 1e-6,
                               fermentable: Fermentable=FERMENTABLES['water']) -> float:
        ...
        :param fermentable: the Fermentable used for dilution (default is water)
        '''
        if parts_water <= 0:
        ...
        predicted_diluted_ph = Must.ph_of_mixture(
            ...
            parts_water, fermentable.ph, fermentable.pka, fermentable.c_buf, tol=tol,
        )
